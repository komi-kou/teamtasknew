...
**原因と解決策**:
- **Socket.io未接続**: ブラウザのコンソールで接続状態を確認
- **チームID不一致**: 同じチームコードでログインしているか確認
- **マルチインスタンス問題**: Redis URLが設定されているか確認（Renderは複数インスタンスを起動する可能性がある）
